...
init.GPS_ODR=104*8/120;
%GPS output data rate in Hz
...
init.IMU0_MAG_ODR= 832.0/12.0;
%Mag output data rate in Hz also used for the temp sensor
init.IMU0_MAG_ZGAUSS= zeros([3 1]);
%gauss applied to get 0 output
init.IMU0_MAG_RMS= zeros([3 1]);%transpose([3.2e-3,3.2e-3,4.1e-3]);
%RMS noise in gauss
init.IMU0_G_ODR= 1664;
%Gyro output data rate in Hz
init.IMU0_G_TYOFF= transpose([0.05,0.05,0.05]);
...
init.IMU0_G_RMS= transpose([5e-3,5e-3,5e-3]);
%RMS noise in rad/s